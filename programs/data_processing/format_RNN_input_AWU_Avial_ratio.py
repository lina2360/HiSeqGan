...
df_source = df[[date_column, target, 'AWU_Avial_ratio']]
...
for target_name in targets:
    ...
    for date in dates:
        ...
        df_sequence.rename(columns={'AWU_Avial_ratio': date}, inplace=True)
...
result.to_csv('./applications/%s/data/tmp_integer_AWU_Avial_ratio.csv' % (prefix), index=False)

df = pd.read_csv('./applications/%s/data/tmp_integer_AWU_Avial_ratio.csv' % (prefix), low_memory=False)
print(df.columns)
...
df.to_csv('./applications/%s/data/rnn_input_data_AWU_Avial_ratio.csv'  % (prefix), index=False)
