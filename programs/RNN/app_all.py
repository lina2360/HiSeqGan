...
input_file_path = ''
test_file_path = './applications/%s/data/rnn_input_item_seq_with_cluster_integer.csv' % prefix
if input_type =='rnn':
...
final_train_results = np.zeros((1,(layer)))
final_results = np.zeros((1,(layer)))
# final_results = np.zeros((1,3))

df_nominal = df.loc[:, ['id','clustered_label']]
df_input = df.drop(['id', 'clustered_label'], axis=1)
df_input_test = df_test.drop(['id', 'clustered_label'], axis=1)
...
df_nominal = df.loc[:, ['id','clustered_label']]
satisify_input_ornot, test_sequence, max_sequence_length, min_sequence_length, df_nominal = format_row_to_layerformat(df_input_test, base_length, layer,number_of_digits,df_nominal)

df_nominal = df_nominal.reset_index()
# df_nominal = df_drop_not_satisified.loc[:, ['clustered_label']]
if satisify_input_ornot:
    print("===== input sequence =====")
    # print(input_sequence)

    # ======== Training and Testing Part ===========
    RNN = WPG_RNN(input_sequence,test_sequence,max_sequence_length, min_sequence_length, 200, 0.01, 5000, 0.01, 1, 0, len(number_of_digits),prefix,output_format)
...
for i in range(0,train_result_df.shape[1]):
    print('train accuracy:',sum(train_result_df[i]==1)/train_result_df.shape[0])
result = final_results[final_results['id']!='none'].drop(['index','id', 'clustered_label'], axis=1)
for i in range(0,result.shape[1]):
    print('test accuracy:',sum(result[i]==1)/result.shape[0])
