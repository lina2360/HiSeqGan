...
class WPG_RNN(object):
    ...
    def __init__(self, df, max_sequence_length, min_sequence_length, num_neurons, learning_rate, max_num_train_iterations, accept_loss, batch_size, training_start_point, layer, prefix,output_format):
        ...
        # layer1 row index
        self._l1_index = np.arange(0, df.shape[0], layer + 1) # 4 = layer + 1
        # layer2 row index
        self._l2_index = np.arange(1, df.shape[0], layer + 1)
        # layer3 row index
        self._l3_index = np.arange(2, df.shape[0], layer + 1)
        # layer4 row index
        self._zero_index = np.arange(3, df.shape[0], layer + 1)     
        self._prefix = prefix
        ...
        with self._graph.as_default():
            ...
            self._loss = tf.reduce_mean(loss_value + tf.reduce_mean(tf.square(zero_diff))*1000)
            # self._loss = tf.reduce_mean(tf.reduce_mean(tf.square(l1_diff))*10000 + tf.reduce_mean(tf.square(l2_diff))*100 + tf.reduce_mean(tf.square(l3_diff))*10 + tf.reduce_mean(tf.square(zero_diff))*10000)
            # def check_l2 ():
    ...
    def test(self, input_vects):
        ...
        with tf.Session() as sess:
            ...
            for i in range(0,new_data.shape[1]):
                print(new_data[0][i])
                # new_array = new_data[0][i]
                new_array = np.insert(new_data[0][i],new_data.shape[2],y_pred[0][i][new_data.shape[2]-1])
                new_array = np.delete(new_array, 0)
                # print('new_array:',new_array)
                new_data[0][i] = new_array
        ...
        accuracy,predict_data = self.model_accuracy(true_value, pred_result)
        print('----------Accuracy----------')
        return accuracy,new_data,predict_data

    def test_multi(self, new_data):
        # RUN TRAINED MODEL AND ESTIMATED RESULT
        with tf.Session() as sess:
            # print('*****************************')
            self._saver.restore(self._sess, "./applications/%s/RNN/model/high_all_wMSE_layers_%s" % (self._prefix,self._output_format))
    ...
    def model_accuracy(self, true_df, pred_array):
        ...
        pred_array = np.absolute(np.squeeze(pred_array, axis=0)[:, -1]).reshape(-1, ((self._layer+1)))
        # pred_array = pred_array.astype(float32)
        print('===== Pred Array information =====')
        print(pred_array)
        print(pred_array.shape)
        true_array = true_df.as_matrix().reshape(-1, (self._layer+1))
        ...
        for j in range(0,len(pred_array)):
            cluster_label = ''
            for i in range(len(pred_array[j])-1):
                cluster_label = cluster_label + str(int(pred_array[j][i]))
            pred_list.append((np.append([], cluster_label)).tolist())
            # pred_list.append(row)
        print('===================pred_array test================')
        print(pred_list)
        # =====
        ...
        # print(result)
        return difference,np.asarray(pred_list)
